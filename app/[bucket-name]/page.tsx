...
import { getMockFiles, DEMO_BUCKET_NAME } from "./mock-data";
...
export default function BucketFileManager() {
  ...
  // Check if this is the demo bucket
  const isDemoBucket = decodeURIComponent(bucketName) === DEMO_BUCKET_NAME;

  // Check if bucket exists in localStorage
  useEffect(() => {
    const checkBucketExists = () => {
      try {
        // If it's the demo bucket, allow access without localStorage entry
        if (isDemoBucket) {
  ...
  }, [bucketName, isDemoBucket]);

  // Utility function to refresh files
  const refreshFiles = async () => {
    if (!bucketCredentials) return;

    // For demo bucket, use mock data
    if (isDemoBucket) {
      setAllFiles(getMockFiles(currentPath));
      return;
    }
  ...
  // Fetch files from S3 API (or mock data for demo bucket)
  useEffect(() => {
    ...
    fetchFiles();
  }, [bucketCredentials, currentPath, isDemoBucket]);
  ...
  // If bucket doesn't exist, this won't render as notFound() is called
  if (!bucketExists) {
    return null;
  }
